...
func runExperiment(x []complex128, n int,
	...
	largeFreq []int, k int, xF []complex128) (float64, float64, float64) {
	if VERBOSE {
	...
	var DDD float64
	var sfftTime float64
	var fftwTime float64
	var errorPerEntry float64
	for i := 0; i < repetitions; i++ {
		//reset_timer();
		DDD = (float64)(time.Now().UnixNano())
		...
		sfftTime = (float64)(time.Now().UnixNano()) - DDD
	...
	sort.Sort(listOfFloats(candidates))
	for i := 0; i < k; i++ {
	...
	for i := 0; i < repetitions; i++ {
		...
		DDD = (float64)(time.Now().UnixNano())
		p.Execute()
		fftwTime = (float64)(time.Now().UnixNano()) - DDD
	}
	if VERBOSE {
	...
	return sfftTime, fftwTime, errorPerEntry
}
